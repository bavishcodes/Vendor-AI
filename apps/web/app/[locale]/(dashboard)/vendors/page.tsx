...
export default function VendorsPage() {
    ...
    }, [vendors, searchTerm, statusFilter, categoryFilter, locationFilter]);
        ...
        <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-light-green/30 overflow-hidden">
                <div className="p-4 border-b border-light-green/30 flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2 px-3 py-2 border border-light-green/30 rounded-lg text-sm font-medium text-charcoal/70 bg-light-green/5">
                        <Filter className="w-4 h-4" />
                        Filters
                    </div>
                    <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as "All" | "Active" | "Warning" | "Inactive")} className="px-3 py-2 border border-light-green/30 rounded-lg text-sm bg-white">
                        <option value="All">All Status</option>
                        <option value="Active">Active</option>
                        <option value="Warning">Warning</option>
                        <option value="Inactive">Inactive</option>
                    </select>
                    <select value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)} className="px-3 py-2 border border-light-green/30 rounded-lg text-sm bg-white">
                        <option value="All">All Category</option>
                        {Array.from(new Set(vendors.map((vendor) => vendor.category))).map((category) => (
                            <option key={category} value={category}>{category}</option>
                        ))}
                    </select>
                    <select value={locationFilter} onChange={(e) => setLocationFilter(e.target.value)} className="px-3 py-2 border border-light-green/30 rounded-lg text-sm bg-white">
                        <option value="All">All Location</option>
                        {Array.from(new Set(vendors.map((vendor) => vendor.location))).map((location) => (
                ...
                <div className="p-4 border-t border-light-green/30 flex items-center justify-between text-sm text-charcoal/50 bg-light-green/5">
                    <div>Showing {filteredVendors.length} of {vendors.length} vendors</div>
