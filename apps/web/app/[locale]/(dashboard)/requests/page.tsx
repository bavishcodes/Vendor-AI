...
import { Clock, CheckCircle2, Package, MapPin, IndianRupee, AlertCircle, Edit3, X } from "lucide-react";
import { useMemo, useState } from "react";
import { useAuth } from "@/context/AuthContext";
import { useSearchParams } from "next/navigation";

const REQUEST_STATUSES: StockRequest["status"][] = ["Approved", "Modified", "Rejected", "In Transit", "Delivered"];
...
export default function AdminRequestsPage() {
    ...
    const [statusFilter, setStatusFilter] = useState<"All" | StockRequest["status"]>(initialStatusFilter);
    ...
    const handleAdminSubmit = (e: React.FormEvent) => {
        ...
        if (selectedRequest) {
            updateRequest(selectedRequest.id, {
                ...
                handledBy: user?.name || "Admin",
                lastAction: `Request ${adminForm.status.toLowerCase()} by ${user?.name || "admin"}`,
            });
    ...
    const getSlaMeta = (request: StockRequest) => {
        const now = new Date();
        const createdAt = new Date(request.createdAt);
        const deadline = new Date(createdAt.getTime() + request.slaHours * 60 * 60 * 1000);
        const remainingMs = deadline.getTime() - now.getTime();
        const isClosed = ["Delivered", "Rejected", "Cancelled"].includes(request.status);
        const isLate = !isClosed && remainingMs < 0;

        let urgency: "High" | "Medium" | "Low" = "Low";
        if (isLate || remainingMs <= 4 * 60 * 60 * 1000) {
            urgency = "High";
        ...
        const label = isClosed ? "Completed" : isLate ? `Late by ${timeText}` : `${timeText} left`;

        return { deadline, remainingMs, isLate, urgency, label };
    };
    ...
    const filteredRequests = useMemo(() => {
        return requests.filter((request) => {
            ...
            const matchesVendor = vendorFilter === "All" || (request.vendorName || "Unknown Vendor") === vendorFilter;
            const matchesUrgency = urgencyFilter === "All" || getSlaMeta(request).urgency === urgencyFilter;
            ...
            const matchesDate =
                ...
                (dateFilter === "Last 7 Days" && created >= sevenDaysAgo) ||
                (dateFilter === "Last 30 Days" && created >= thirtyDaysAgo);

            return matchesQuery && matchesStatus && matchesVendor && matchesUrgency && matchesDate;
        });
    }, [requests, searchTerm, statusFilter, vendorFilter, urgencyFilter, dateFilter]);

    const lateCount = requests.filter((request) => getSlaMeta(request).isLate).length;
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-light-green/30 p-4 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-3">
                <input
                    type="text"
                    placeholder="Search by request, product, vendor"
                    value={searchTerm}
                    onChange={(e) => setSearchTerm(e.target.value)}
                    className="w-full px-3 py-2 border border-light-green/30 rounded-lg text-sm outline-none focus:ring-2 focus:ring-deep-green/20"
                />
                <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as "All" | StockRequest["status"])} className="px-3 py-2 border border-light-green/30 rounded-lg text-sm bg-white">
                ...
                <select value={vendorFilter} onChange={(e) => setVendorFilter(e.target.value)} className="px-3 py-2 border border-light-green/30 rounded-lg text-sm bg-white">
                    <option value="All">All Vendors</option>
                    {Array.from(new Set(requests.map((request) => request.vendorName || "Unknown Vendor"))).map((name) => (
                        <option key={name} value={name}>{name}</option>
                    ))}
                </select>
                <select value={urgencyFilter} onChange={(e) => setUrgencyFilter(e.target.value as "All" | "High" | "Medium" | "Low")} className="px-3 py-2 border border-light-green/30 rounded-lg text-sm bg-white">
                    <option value="All">All Urgency</option>
                    <option value="High">High</option>
                    <option value="Medium">Medium</option>
                    <option value="Low">Low</option>
                </select>
                <select value={dateFilter} onChange={(e) => setDateFilter(e.target.value as "All" | "Today" | "Last 7 Days" | "Last 30 Days")} className="px-3 py-2 border border-light-green/30 rounded-lg text-sm bg-white">
                    ...
                    <table className="w-full text-left text-sm">
                        <thead className="bg-light-green/10 text-charcoal/70 border-b border-light-green/20">
                            <tr>
                                ...
                                <th className="px-6 py-4 font-bold">Lifecycle</th>
                        ...
                        <tbody className="divide-y divide-light-green/10">
                            {filteredRequests.length > 0 ? (
                                    ...
                                    <tr key={request.id} className="hover:bg-light-green/5 transition-colors group">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex flex-col gap-1">
                                                <span className={`inline-flex items-center px-2 py-1 rounded-full text-[10px] font-black uppercase border w-fit ${sla.urgency === "High" ? "bg-red-50 text-red-700 border-red-200" : sla.urgency === "Medium" ? "bg-amber-50 text-amber-700 border-amber-200" : "bg-emerald-50 text-emerald-700 border-emerald-200"}`}>
                                                    {sla.urgency} Priority
                                                </span>
                                                <span className={`text-xs font-semibold ${sla.isLate ? "text-red-600" : "text-charcoal/60"}`}>{sla.label}</span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 text-xs text-charcoal/60 min-w-[210px]">
                                            <div className="space-y-1">
                                                <p><span className="font-semibold">Created:</span> {new Date(request.createdAt).toLocaleString()}</p>
                                                <p><span className="font-semibold">Assigned:</span> {request.assignedAt ? new Date(request.assignedAt).toLocaleString() : "-"}</p>
                                                <p><span className="font-semibold">Delivered:</span> {request.deliveredAt ? new Date(request.deliveredAt).toLocaleString() : "-"}</p>
                                                <p><span className="font-semibold">Last:</span> {request.lastAction} {request.handledBy ? `by ${request.handledBy}` : ""}</p>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4">
                                            <div className="flex gap-2">
                                                ...
                                                {request.status !== "Delivered" && (
                                                    <button
                                                        onClick={() => updateRequest(request.id, {
                                                            status: "Delivered",
                                                            handledBy: user?.name || "Admin",
                                                            lastAction: `Marked delivered by ${user?.name || "admin"}`,
                                                        })}
                                ...
                                <tr>
                                    <td colSpan={8} className="px-6 py-12 text-center text-charcoal/40">
                                        No stock requests found for selected filters.
                                    </td>
                                </tr>
                            )}
                        </tbody>
